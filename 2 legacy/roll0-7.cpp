...
/*
draft seven swinging relatively sightedly in the dark to do hour change animations
bgilder
4jan2017 
*/

/****ATTENTION!!!!! animations have been moved to main file as of rev0-15!!!
*/

/******* rev log ********
7
updated to work with the mux enable array
added fancy new animation - swipe_left
...
void swipe_left()		//flies old time out the left of the clock and new time in from the right
{
	...
	{			
		canDo=0;
		while(!canDo)	//whatever. sort out magic number later	//for set, this can just sit at 10, right? or maybe not for hr1 and hr0 hm. maybe at the end of this, we reset magic number to h1,hr0,10,10,10,10
		{
			mux(roll_buffer, mux_enable);
			currentSec = millis();
	...
	for(int go=clockDigitSize; go>0; go--)	//this makes the new numbers fly in from the right	//should this be go > 1 so it ends and has the regular mux do the normal ending frame?
